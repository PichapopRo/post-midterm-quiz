...
my_table1_filtered = my_table1.filter(lambda x: x['Genre'] == 'Comedy').filter(
    lambda x: x['Worldwide Gross'])
avg_list = []
for i in my_table1_filtered.table:
    avg_list.append(float(i['Worldwide Gross']))
result = sum(avg_list) / len(avg_list)
print(f"Average Worldwide Gross : {result:.2f}")

my_table1_filtered2 = my_table1.filter(lambda x: x['Genre'] == 'Drama').filter(
    lambda x: x['Audience score %'])
percent_list = []
for i in my_table1_filtered2.table:
    percent_list.append(float(i['Audience score %']))
print(f"minimum Audience score %: {min(percent_list)}")
